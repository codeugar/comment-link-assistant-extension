...
列表支持搜索、按属性筛选、分页、新增、编辑和删除。每个站点以规范化后的域名去重，输入 `example.com`、`www.example.com/post` 或完整 HTTP(S) URL 都会归到同一个域名。
...
其他列不存在时保持“未知”，存在时按顺序读取。导入前会显示有效与无效行预览；重复域名会更新已有记录，不会创建重复项。页面也提供标准 CSV 模板下载。
